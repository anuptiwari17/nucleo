          ...
          <div className="space-y-2 text-sm text-white/80 mb-4">
            ...
            {task.completedDate && (
              <div className="flex items-center space-x-2">
                ...
                <span><span className="font-medium">Completed:</span> {formatDate(task.completedDate)}</span>
              </div>
            )}
            {task.dueDate && (
              <div className="flex items-center space-x-2">
                ...
                {task.completedDate && (
                  <span className={`ml-2 px-2 py-1 rounded-full text-xs font-medium ${
                    completedOnTime ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'
                  }`}>
      ...
      <div className="bg-gradient-to-r from-green-500 to-teal-600 p-6">
        <div className="flex items-center space-x-3 text-white">
          ...
          <div>
            ...
            <p className="text-white/80 text-sm">{tasks.length} tasks completed</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6">
        {tasks.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-white/80 text-lg">No completed tasks yet</p>
            <p className="text-white/60 text-sm mt-1">Complete active tasks to see them here</p>
          </div>
        ) : (
